...
class TOperator_base {
 public:
  ...
  void SetInit();
  MatrixXd Trans_AtoB( const MatrixXd& A ); // return matrix B from matrix A (Eq. 3 and 4)
 ...
 // set matrix A 
  ...
  void Make_Bv_Be_IH27();
  void Make_B_IH27( int k1, int k2 );
  ...
  MatrixXd fBv;  // (B_v in Eq. 15)
  int fMv;       // = fBv.cols() 
  MatrixXd fBe;  // The row elements store {d^b_s}^T and {d^f_s}^T (Eq. 22)
  MatrixXd fBd;  // dens part of the matrix B (B''_d in Eq. 24)
  ...
  int *fn; // fn[v] (v=0, ..., fNv-1) the numbers of points on the tiling edges of a template (k_1, k_2, ... )
   ...
   int inline ix( int i ){ // positoin of the x-coordinate of the i-th point of the tile polygon
    if( i < 0 )
      i += fN;
    else if ( i >= fN )
      i -= fN;
  ...
  int inline iy( int i ){ // positoin of the x-coordinate of the i-th point of the tile polygon
    if( i < 0 )
      i += fN;
    else if ( i >= fN )
      i -= fN;
